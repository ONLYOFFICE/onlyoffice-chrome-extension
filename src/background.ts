...
import { OAUTH_CLIENT_ID, OAUTH_SCOPES, TOKEN_EXCHANGE_URL, validateConfig } from '@config';
...
async function exchangeCode(code: string) {
    try {
        ...
        chrome.runtime.sendMessage({
            action: 'oauthSuccess',
            tokens: authData
        }).catch(err => console.log('Could not notify popup:', err.message));
    } catch (error) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'startOAuthFlow') {
        ...
        let redirectUri = chrome.identity.getRedirectURL().replace(/\/$/, '');
    ...
    if (request.action === 'uploadFile') {
        (async () => {
            try {
                const { tenant, accessToken, fileData, fileName, fileType } = request;
